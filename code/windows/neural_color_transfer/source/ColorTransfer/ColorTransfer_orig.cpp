...
#pragma omp parallel for
	for (int i = 0; i < sz; ++i)
	{
		...
		//const Vec3b& col = subCluster[i].col;
		//colorPnts.pts[i] = Vec3f(float(col[0]) * factor, float(col[1]) * factor, float(col[2]) * factor);
		//int id = subCluster[i].id;
		//float x = int(id % width) / (float)width;
		//float y = int(id / width) / (float)height;
		//colorPnts.pts[i] = Vec6f(float(col[0]) * factor, float(col[1]) * factor, float(col[2]) * factor, x, y, 0.f);
	}
...
void ColorTransfer::clusterFeastures(Mat& dvisMat, float* features, int width, int height, int channel)
{
	...
	printf("HMM: before Cluster Num = %d\n", cNum);
	...
	printf("HMM: after Cluster Num = %d\n", m_labelNum);
#endif
...
#pragma omp parallel for
	...
	// covert distance to weight by Softmax
	/*if (0)
	{
		for (int i = 0; i < m_labelNum; ++i)
		{
			findSubKNNs(subClusterPixels[i], cntLab.cols, cntLab.rows);
		}
...
//		}
	...
	//float xRatio = (float)dwidth / (float)width;
	//float yRatio = (float)dheight / (float)height;

	for (int y = 0; y < height; ++y)
	{
		/*float dy = y * yRatio;
		int y0 = floor(dy);
		int y1 = min(y0 + 1, dheight - 1);
		float wy0 = y1 - dy;
		float wy1 = 1 - wy0;*/
		...
		for (int x = 0; x < width; ++x)
		{
			/*float dx = x * xRatio;
			int x0 = floor(dx);
			int x1 = min(x0 + 1, dwidth - 1);
			float wx0 = x1 - dx;
			float wx1 = 1 - wx0;

			const Vec3d& a00 = daMat.at<Vec3d>(y0, x0);
			const Vec3d& a01 = daMat.at<Vec3d>(y0, x1);
			const Vec3d& a10 = daMat.at<Vec3d>(y1, x0);
			const Vec3d& a11 = daMat.at<Vec3d>(y1, x1);

			const Vec3d& b00 = dbMat.at<Vec3d>(y0, x0);
			const Vec3d& b01 = dbMat.at<Vec3d>(y0, x1);
			const Vec3d& b10 = dbMat.at<Vec3d>(y1, x0);
			const Vec3d& b11 = dbMat.at<Vec3d>(y1, x1);
			...
			float w11 = wy1 * wx1;
			Vec3d a = Vec3d(
				a00[0] * w00 + a01[0] * w01 + a10[0] * w10 + a11[0] * w11,
				a00[1] * w00 + a01[1] * w01 + a10[1] * w10 + a11[1] * w11,
				a00[2] * w00 + a01[2] * w01 + a10[2] * w10 + a11[2] * w11);

			Vec3d b = Vec3d(
				b00[0] * w00 + b01[0] * w01 + b10[0] * w10 + b11[0] * w11,
				b00[1] * w00 + b01[1] * w01 + b10[1] * w10 + b11[1] * w11,
				b00[2] * w00 + b01[2] * w01 + b10[2] * w10 + b11[2] * w11);
...
#pragma omp parallel for
		for (int y = 0; y < m_height; ++y)
		{
			for (int x = 0; x < m_width; ++x)
			{
				...
				const Vec3d& val0 = m_cntLabD.at<Vec3d>(y, x);
				if (x + 1 < m_width)
				{
					const Vec3d& val1 = m_cntLabD.at<Vec3d>(y, x + 1);
					double gx = sqrt((val1[0] - val0[0]) * (val1[0] - val0[0]) + 
						(val1[1] - val0[1]) * (val1[1] - val0[1]) +
						(val1[2] - val0[2]) * (val1[2] - val0[2])) / 3;
					m_gradX.at<double>(y, x) = sqrt(lamda / (pow(abs(gx), alpha) + epsilon));
				}
				if (y + 1 < m_height)
				{
					const Vec3d& val1 = m_cntLabD.at<Vec3d>(y + 1, x);
					double gy = sqrt((val1[0] - val0[0]) * (val1[0] - val0[0]) +
						(val1[1] - val0[1]) * (val1[1] - val0[1]) +
						(val1[2] - val0[2]) * (val1[2] - val0[2])) / 3;
					m_gradY.at<double>(y, x) = sqrt(lamda / (pow(abs(gy), alpha) + epsilon));
				}
...
#pragma omp parallel for
		for (int y = 0; y < height; ++y)
		{
			for (int x = 0; x < width; ++x)
			{
				const Vec3d& val0 = cntLabD.at<Vec3d>(y, x);
				gradX.at<double>(y, x) = 0;
				gradY.at<double>(y, x) = 0;
				if (x + 1 < width)
				{
					const Vec3d& val1 = cntLabD.at<Vec3d>(y, x + 1);
					double gx = sqrt((val1[0] - val0[0]) * (val1[0] - val0[0]) +
						(val1[1] - val0[1]) * (val1[1] - val0[1]) +
						(val1[2] - val0[2]) * (val1[2] - val0[2])) / 3;
				...
				if (y + 1 < height)
				{
					const Vec3d& val1 = cntLabD.at<Vec3d>(y + 1, x);
					double gy = sqrt((val1[0] - val0[0]) * (val1[0] - val0[0]) +
						(val1[1] - val0[1]) * (val1[1] - val0[1]) +
						(val1[2] - val0[2]) * (val1[2] - val0[2])) / 3;
	...
	const Mat& cntRgb, int layer, float lambda, float alpha, float dWeight)
{
	...
	//double localWeight = sqrt(m_config.m_localWeight);
	...
	// HMM@TEST
	//int localConstraints = size * (patchSize * patchSize - 1) * 2;
	int localConstraints = size * 4 * 2;// size * 4 * 2;
	...
	// HMM@TEST
	//dWeight *= 700.0 * 500.0 / float(m_width * m_height);
	...
	if (1)
	{
		for (int y = 0; y < height; y++)
		{
			for (int x = 0; x < width; x++)
			{
				...
				{
					const double gxw = gradX.at<double>(y, x) /** localWeight*/;

					/*if (gxw > 999999.f || gxw < -999999.f)
					{
						int hmm_test = 0;
						hmm_test++;
					}*/
				...
				if (x - 1 >= 0)
				{
					const double gxw = gradX.at<double>(y, x - 1) /** localWeight*/;
					CBab0[cid] = 0.0;
					CBab1[cid] = 0.0;
					CBab2[cid] = 0.0;
				...
				{
					const double gyw = gradY.at<double>(y, x) /** localWeight*/;
				...
				if (y - 1 >= 0)
				{
					const double gyw = gradY.at<double>(y - 1, x) /** localWeight*/;
					CBab0[cid] = 0.0;
					CBab1[cid] = 0.0;
					CBab2[cid] = 0.0;
	...
	if (1)
	{
		for (int c = 0; c < m_knnid.size(); ++c)
		{
			...
			for (int ki = 0; ki < num; ++ki)
			{
				...
				CA0[nNonZeros] = iw;// *sVec0[0];
				CA1[nNonZeros] = iw;// *sVec0[1];
				CA2[nNonZeros] = iw;// *sVec0[2];
				Ccolumns[nNonZeros] = min(aii0, aii1) + oneBased;
				nNonZeros++;

				CA0[nNonZeros] = -iw;// *sVec1[0];
				CA1[nNonZeros] = -iw;// *sVec1[1];
				CA2[nNonZeros] = -iw;// *sVec1[2];
...
void ColorTransfer::solve_WLS_roughness_cpu(const vector<double>& roughness, Mat& aMat, Mat& bMat, double alpha, double lamda, int downWidth, int downHeight)
{
	...
	//float xratio = (float)m_width / (float)downWidth;
	//float yratio = (float)m_height / (float)downHeight;
	...
	for (int y = 0; y < m_height; y++)
	{
		for (int x = 0; x < m_width; x++)
		{
			...
			// data term
			double w = roughness[y * m_width + x];//[dy * downWidth + dx];// sqrt(weight.at<double>(y, x)); //1.f;
			const Vec3d& aVec = aMat.at<Vec3d>(y, x);
			const Vec3d& bVec = bMat.at<Vec3d>(y, x);
			...
			if (a00 <= 1)
			{
				printf("HMM: nonzeros = %d, a00 = %f, x = %d, y = %d\n", nNonZeros, a00, x, y);
			}
...
#pragma omp parallel for
	for (int y = 0; y < height; ++y)
	{
		for (int x = 0; x < width; ++x)
		{
			...
			aVec[0] = stlVarVal[0] / (cntVarVal[0] + m_config.m_varEpslon);// max(min(stlVarVal[0] / (cntVarVal[0] + m_config.m_varEpslon), 20.0), 0.05);
...
#pragma omp parallel for
	...
	//double factor = 0.25;// m_config.m_wlsLamdaFactor;
	double lamda = initVal * normFactor; //initVal * pow(factor, 4) * normFactor;//initVal * pow(factor, layer);
	printf("HMM: layer = %d, lamda =  %lf\n", layer, lamda);
...
#pragma omp parallel for
		...
		// HMM@TEST
		solve_WLS_roughness_cpu(roughness, m_a, m_b, m_config.m_wlsAlpha, lamda, downWidth, downHeight);
